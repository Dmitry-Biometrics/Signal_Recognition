...
void QCustomPlot::movePlottable(int ifrom, int ito){
    Q_ASSERT(ifrom>=0 && ifrom<mPlottables.count());
    Q_ASSERT(ito>=0 && ito<=mPlottables.count());
    if (ifrom==ito) return;
    QCPAbstractPlottable* p   = mPlottables[ifrom];
    QList<QCPLayerable*> list = p->layer()->children();
    //Найти позиции в слое, соответствующие списку графиков
    int lfrom=0, lto=0;
    for (int i=0; i<list.count(); ++i)
        if (list[i]==p) { lfrom=i; break; }
    if (ito==mPlottables.count())
        lto = list.count(); //в конец
    else
        for (int i=0; i<list.count(); ++i) //в соотв. позицию
            if (list[i]==mPlottables[ito]) { lto=i; break; }
    //qDebug()<<"i "<<ifrom<<' '<<ito<<" l"<<lfrom<<' '<<lto;
    if (ito>ifrom) ito--;
    mPlottables.insert(ito,mPlottables.takeAt(ifrom));
    p->layer()->move(lfrom,lto);
}
