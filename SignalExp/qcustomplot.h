...
public:
  ...
  void move(int ifrom, int ito){
      Q_ASSERT(ifrom>=0 && ifrom<mChildren.count());
      Q_ASSERT(ito>=0 && ito<=mChildren.count());
      if (ito>ifrom) --ito;//после удаления элемента позиция ito поменяется
      mChildren.insert(ito,mChildren.takeAt(ifrom));
  }
...
public:
  ...
  */
  ...
  void movePlottable(int ifrom, int ito);
